...
### 가변 / 불변 구분짓는 변수할당[var / val](https://velog.io/@jojo_devstory/Kotlin-val-var%EC%9D%98-%EC%B0%A8%EC%9D%B4%EC%A0%90)
+ var  
  1. var 로 선언한 변수는 값을 변경할 수 있다. var로 지정된 가변 타입 변수는 초기화 후 값의 변경이 가능하다는 소리이다. 하지만 다른 타입의 값을 넣을 수는 없다.  
  2. 처음에 String타입으로 초기화 한뒤 Int 타입의 값을 넣을 수 없다. 이미 정의된 타입을 변경하려면 자바와 같이 형 변환이 필요하다.  
```KOTLIN
...
```
+ val  
  1. val 로 선언한 변수는 값을 변경할 수 없다[]. 초기화만 가능함(Java에서의 final)  
  2. val 은 한번만 초기화 되며 값을 변경할 수 없지만 컴파일러가 코드 문맥을 확인시 val변수가 한번만 초기화 되는것이 맞다면 개발자가 상황에 따라 val값을 여러 값으로 초기화(할당) 할 수 있다.  
  3. val은 초기화 후 값을 변경하는것이 불가능하지만, 변수의 참조가 가리키는 객체의 내부 값은 변경이 가능하다.
```KOTLIN
fun main(args: Array<String>){
